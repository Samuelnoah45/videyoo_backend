...
func VerifyProjectStockOutTransaction(ctx *gin.Context) {
	...
	projectStockOutTransaction, dataFetchError := transactionService.GetProjectStockOutTransaction(inputData.Transaction_id)
	...
	if !projectStockOutTransaction.Is_verified {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": "Transaction is already verified"})
		return
	}

	// check if verification code is correct
	if !transactionService.ValidatedVerificationCode(inputData.VerificationCode, projectStockOutTransaction.Transaction_verification_code) {
	...
	err := transactionService.UpdateProjectStockOutTransaction(inputData.Transaction_id, true)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		return
	}
	// modify the warehouse
	transaction, err := transactionService.GetTransaction(projectStockOutTransaction.Transaction_id, xHasuraRole, tokenString)
	if err != nil {
		ctx.JSON(http.StatusBadRequest, gin.H{"error": err.Error()})
		transactionService.UpdateProjectStockOutTransaction(inputData.Transaction_id, false)
	}
